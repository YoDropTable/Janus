...
public sealed class SqliteJanusRepository(string databasePath) : IJanusRepository
{
    ...
    public async Task<Asset?> GetAssetAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        await using var connection = await OpenConnectionAsync(cancellationToken);
        await using var command = connection.CreateCommand();
        command.CommandText = "SELECT id, name, type, manufacturer, model, serial_number, description, created_at, updated_at FROM assets WHERE id = $id";
    ...
    private async Task<IReadOnlyList<Asset>> GetAllAssetsAsync(CancellationToken cancellationToken)
    {
        await using var connection = await OpenConnectionAsync(cancellationToken);
        await using var command = connection.CreateCommand();
        command.CommandText = "SELECT id, name, type, manufacturer, model, serial_number, description, created_at, updated_at FROM assets ORDER BY name COLLATE NOCASE";
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        command.CommandText = isUpdate
            ? """
              UPDATE assets SET name = $name, type = $type, manufacturer = $manufacturer,
                  model = $model, serial_number = $serialNumber, description = $description,
                  updated_at = $updatedAt
              WHERE id = $id
              """
            : """
              INSERT INTO assets (id, name, type, manufacturer, model, serial_number, description, created_at, updated_at)
              VALUES ($id, $name, $type, $manufacturer, $model, $serialNumber, $description, $createdAt, $updatedAt)
              """;
    ...
    private static Asset ReadAsset(SqliteDataReader reader) => new()
    {
        ...
        Type = GetNullableString(reader, 2),
        Manufacturer = GetNullableString(reader, 3),
        Model = GetNullableString(reader, 4),
        SerialNumber = GetNullableString(reader, 5),
